...
const SIGNUP_PHONE_PASSWORD_MISMATCH_PATTERN = /incorrect\s+phone\s+number\s+or\s+password|phone\s+number\s+or\s+password|failed\s+to\s+create\s+account|couldn'?t\s+create\s+(?:your\s+)?account|unable\s+to\s+create\s+(?:your\s+)?account|与此(?:电话|手机)号码相关联的帐户已存在|account\s+associated\s+with\s+this\s+phone\s+number\s+already\s+exists|创建帐户失败|无法创建(?:你的|您的)?帐户/i;
