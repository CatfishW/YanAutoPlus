...
function getPhoneSmsProviderLabel(provider = getSelectedPhoneSmsProvider()) {
  ...
  return normalized === PHONE_SMS_PROVIDER_FIVE_SIM ? '5sim' : 'HeroSMS';
}
...
function updateHeroSmsPlatformDisplay() {
  ...
  const selected = provider === PHONE_SMS_PROVIDER_SMSPOOL
  ...
  const serviceText = provider === PHONE_SMS_PROVIDER_SMSPOOL
  ...
  if (inputHeroSmsApiKey) {
    inputHeroSmsApiKey.placeholder = provider === PHONE_SMS_PROVIDER_SMSPOOL
      ? '请输入 SMSPool API Key'
      : provider === PHONE_SMS_PROVIDER_FIVE_SIM
      ? '请输入 5sim API Key'
...
async function previewPhoneSmsBalance() {
  ...
  if (rowHeroSmsPriceTiers) rowHeroSmsPriceTiers.style.display = '';
  try {
    const url = provider === PHONE_SMS_PROVIDER_SMSPOOL
...
function updatePhoneVerificationSettingsUI() {
  ...
  if (rowHeroSmsApiKey) rowHeroSmsApiKey.style.display = showProviderConfig && heroProvider ? '' : 'none';
  if (rowFiveSimApiKey) rowFiveSimApiKey.style.display = showProviderConfig && fiveSimProvider ? '' : 'none';
  if (rowFiveSimCountry) rowFiveSimCountry.style.display = showProviderConfig && fiveSimProvider ? '' : 'none';
  if (rowFiveSimCountryFallback) rowFiveSimCountryFallback.style.display = showProviderConfig && fiveSimProvider ? '' : 'none';
  if (rowFiveSimOperator) rowFiveSimOperator.style.display = showProviderConfig && fiveSimProvider ? '' : 'none';
  if (rowFiveSimProduct) rowFiveSimProduct.style.display = showProviderConfig && fiveSimProvider ? '' : 'none';
  if (rowNexSmsApiKey) rowNexSmsApiKey.style.display = showProviderConfig && nexSmsProvider ? '' : 'none';
  if (rowNexSmsCountry) rowNexSmsCountry.style.display = showProviderConfig && nexSmsProvider ? '' : 'none';
  if (rowNexSmsCountryFallback) rowNexSmsCountryFallback.style.display = showProviderConfig && nexSmsProvider ? '' : 'none';
  if (rowNexSmsServiceCode) rowNexSmsServiceCode.style.display = showProviderConfig && nexSmsProvider ? '' : 'none';
  if (rowFiveSimOperator) {
    rowFiveSimOperator.style.display = showProviderConfig && fiveSimProvider ? '' : 'none';
  }
...
function applySettingsState(state) {
  ...
  if (inputHeroSmsApiKey) {
    inputHeroSmsApiKey.value = restoredPhoneSmsProvider === PHONE_SMS_PROVIDER_SMSPOOL
      ? (state?.smsPoolApiKey || DEFAULT_SMS_POOL_API_KEY)
  ...
  if (typeof applyHeroSmsFallbackSelection === 'function') {
    const primaryCountry = restoredPhoneSmsProvider === PHONE_SMS_PROVIDER_SMSPOOL
      ? {
        id: String(state?.smsPoolCountryId || DEFAULT_SMS_POOL_COUNTRY_ID).trim().toUpperCase(),
    ...
    if (restoredPhoneSmsProvider === PHONE_SMS_PROVIDER_SMSPOOL) {
...
async function switchPhoneSmsProvider(nextProvider) {
  ...
  smsPoolCountrySelectionOrder = [];
  if (inputHeroSmsApiKey) {
    inputHeroSmsApiKey.value = normalizedNextProvider === smsPoolProviderValue
  ...
  const restoredPrimary = normalizedNextProvider === smsPoolProviderValue
    ? {
  ...
  const restoredFallback = normalizedNextProvider === smsPoolProviderValue
    ? (Array.isArray(latestState?.smsPoolCountryFallback) ? latestState.smsPoolCountryFallback : [])
    : normalizedNextProvider === PHONE_SMS_PROVIDER_FIVE_SIM
    ? normalizeFiveSimCountryFallbackList(latestState?.fiveSimCountryFallback || [])
    : normalizeHeroSmsCountryFallbackList(latestState?.heroSmsCountryFallback || []);
