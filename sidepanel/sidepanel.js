...
const preStartSkipControl = document.getElementById('prestart-skip-control');
const preStartSkipOptions = document.getElementById('prestart-skip-options');
const preStartSkipSummary = document.getElementById('prestart-skip-summary');
...
const plusModeCaption = document.getElementById('plus-mode-caption');
...
const rowIcloudAliasLabelPattern = document.getElementById('row-icloud-alias-label-pattern');
...
const rowOutlookAliasPattern = document.getElementById('row-outlook-alias-pattern');
const inputOutlookAliasPattern = document.getElementById('input-outlook-alias-pattern');
...
const rowGmailAliasPattern = document.getElementById('row-gmail-alias-pattern');
const inputGmailAliasPattern = document.getElementById('input-gmail-alias-pattern');
const rowGmailMailboxUrl = document.getElementById('row-gmail-mailbox-url');
const inputGmailMailboxUrl = document.getElementById('input-gmail-mailbox-url');
...
const rowSmsPoolApiKey = document.getElementById('row-sms-pool-api-key');
const rowSmsPoolCountry = document.getElementById('row-sms-pool-country');
...
const inputSmsPoolApiKey = document.getElementById('input-sms-pool-api-key');
...
const selectSmsPoolCountry = document.getElementById('select-sms-pool-country');
const smsPoolCountryMenuShell = document.getElementById('sms-pool-country-menu-shell');
const btnSmsPoolCountryMenu = document.getElementById('btn-sms-pool-country-menu');
const smsPoolCountryMenu = document.getElementById('sms-pool-country-menu');
const btnSmsPoolCountryClear = document.getElementById('btn-sms-pool-country-clear');
...
const GUIDE_REPOSITORY_URL = 'https://github.com/CatfishW/YanAutoPlus';
...
let smsPoolCountrySelectionOrder = [];
let smsPoolCountryMenuSearchKeyword = '';
const smsPoolCountrySearchTextById = new Map();
...
let preStartSkippedNodeIds = new Set();
...
const DEFAULT_PHONE_SMS_PROVIDER = PHONE_SMS_PROVIDER_SMSPOOL;
...
const DEFAULT_SMS_POOL_COUNTRY_ID = 'US';
...
const DEFAULT_SMS_POOL_POOL_ID = '7';
const DEFAULT_SMS_POOL_POOL_LABEL = 'Foxtrot';

function hasChromeRuntimeApi() {
...
function getChromeRuntimeManifest() {
  if (
  ...
  return {
    version_name: 'YanAutoPlus 0.1.5',
    version: '0.1.5',
  };
}

function buildLocalSidepanelPreviewState() {
  return {
    panelMode: DEFAULT_PANEL_MODE,
    phoneVerificationEnabled: DEFAULT_PHONE_VERIFICATION_ENABLED,
    signupMethod: DEFAULT_SIGNUP_METHOD,
    phoneSmsProvider: DEFAULT_PHONE_SMS_PROVIDER,
    phoneSmsProviderOrder: [DEFAULT_PHONE_SMS_PROVIDER],
    ...
    smsPoolCountryOrder: [...DEFAULT_SMS_POOL_COUNTRY_ORDER],
    smsPoolCountryFallback: [],
    smsPoolServiceId: DEFAULT_SMS_POOL_SERVICE_ID,
...
function readPhoneVerificationSectionExpanded() {
  try {
    const stored = window.localStorage?.getItem(PHONE_VERIFICATION_SECTION_EXPANDED_STORAGE_KEY);
    if (stored === '0') {
      return false;
    }
...
function persistPhoneVerificationSectionExpanded(expanded) {
  try {
    if (expanded) {
      ...
      window.localStorage?.setItem(PHONE_VERIFICATION_SECTION_EXPANDED_STORAGE_KEY, '0');
    }
  } catch (err) {
...
const DEFAULT_SMSPOOL_COUNTRY_ID = DEFAULT_SMS_POOL_COUNTRY_ID;
...
const SUPPORTED_IP_PROXY_SERVICES = ['711proxy', 'lumiproxy', 'iproyal', 'omegaproxy', IP_PROXY_SERVICE_CLASH];
const IP_PROXY_ENABLED_SERVICES = ['711proxy', IP_PROXY_SERVICE_CLASH];
...
function buildManagedAliasBaseEmailPayload(state = latestState) {
  const payload = {
    ...
    gmailAliasPattern: String(state?.gmailAliasPattern || '').trim(),
    gmailMailboxUrl: String(state?.gmailMailboxUrl || '').trim(),
    mail2925BaseEmail: String(state?.mail2925BaseEmail || '').trim(),
    mail2925UseAccountPool: Boolean(state?.mail2925UseAccountPool),
    icloudAliasLabelPattern: normalizeIcloudAliasLabelPattern(state?.icloudAliasLabelPattern),
    emailPrefix: '',
  };
  if (inputIcloudAliasLabelPattern) {
    payload.icloudAliasLabelPattern = normalizeIcloudAliasLabelPattern(inputIcloudAliasLabelPattern.value);
  }
  if (inputGmailAliasPattern) {
    payload.gmailAliasPattern = inputGmailAliasPattern.value.trim();
  }
  if (inputGmailMailboxUrl) {
    payload.gmailMailboxUrl = inputGmailMailboxUrl.value.trim();
  }
...
const normalizeIcloudAliasLabelPattern = (value = '') => {
...
function shouldWarnCpaPhoneSignup(signupMethod = getSelectedSignupMethod(), panelMode = getSelectedPanelMode()) {
  if (isCpaPhoneSignupPromptDismissed()) {
    return false;
  }

  const normalizedSignupMethod = normalizeSignupMethod(signupMethod);
  ...
  const capabilityState = registry?.resolveSidepanelCapabilities
    ? registry.resolveSidepanelCapabilities({
      ...
      signupMethod: normalizedSignupMethod,
      state: {
  ...
  if (typeof capabilityState?.shouldWarnCpaPhoneSignup === 'boolean') {
    return capabilityState.shouldWarnCpaPhoneSignup;
  }

  return normalizedSignupMethod === SIGNUP_METHOD_PHONE && normalizedPanelMode === 'cpa';
}

async function openCpaPhoneSignupWarningModal() {
  return openConfirmModalWithOption({
    title: 'CPA 手机号注册提醒',
...
async function confirmCpaPhoneSignupIfNeeded({
...
function normalizeOutlookAliasPattern(value) {
  return String(value || '').trim() || 'PayPal{n}';
}
...
function collectSettingsPayload() {
  ...
  const normalizeHeroSmsCountryFallbackListSafe = typeof normalizeHeroSmsCountryFallbackList === 'function'
    ...
    : ((value = []) => {
      ...
      source.forEach((entry) => {
        const id = Number(entry?.id ?? entry?.countryId ?? entry);
        ...
        normalized.push({
          id,
          label: String(entry?.label || entry?.countryLabel || `Country #${id}`).trim() || `Country #${id}`,
        });
      });
      return normalized;
    });
  ...
  const normalizeIpProxyServiceSafe = typeof normalizeIpProxyService === 'function'
    ...
    : ((value = '') => {
      const normalized = String(value || '').trim().toLowerCase();
      return ['711proxy', 'clash'].includes(normalized)
        ? normalized
        : '711proxy';
    });
  ...
  const phoneSmsProviderFiveSim = typeof PHONE_SMS_PROVIDER_FIVE_SIM !== 'undefined'
    ? PHONE_SMS_PROVIDER_FIVE_SIM
    : '5sim';
  const defaultSmsPoolApiKey = typeof DEFAULT_SMS_POOL_API_KEY !== 'undefined'
    ? DEFAULT_SMS_POOL_API_KEY
    : '';
  ...
  const defaultSmsVerificationNumberServiceCode = typeof DEFAULT_SMS_VERIFICATION_NUMBER_SERVICE_CODE !== 'undefined'
    ...
    : 'dr';
  const defaultSmsPoolCountryId = typeof DEFAULT_SMS_POOL_COUNTRY_ID !== 'undefined'
    ? DEFAULT_SMS_POOL_COUNTRY_ID
    : 'US';
  const defaultSmsPoolCountryLabel = typeof DEFAULT_SMS_POOL_COUNTRY_LABEL !== 'undefined'
    ? DEFAULT_SMS_POOL_COUNTRY_LABEL
    : 'United States';
  const defaultSmsPoolServiceId = typeof DEFAULT_SMS_POOL_SERVICE_ID !== 'undefined'
    ? DEFAULT_SMS_POOL_SERVICE_ID
    : '671';
  const defaultSmsPoolServiceLabel = typeof DEFAULT_SMS_POOL_SERVICE_LABEL !== 'undefined'
    ? DEFAULT_SMS_POOL_SERVICE_LABEL
    : 'OpenAI / ChatGPT';
  const defaultSmsPoolPoolId = typeof DEFAULT_SMS_POOL_POOL_ID !== 'undefined'
    ? DEFAULT_SMS_POOL_POOL_ID
    : '7';
  ...
  const currentPhoneSmsApiKeyValue = phoneSmsProviderValue === phoneSmsProviderSmsPool
    ...
    : (typeof inputHeroSmsApiKey !== 'undefined' && inputHeroSmsApiKey
      ...
      : '');
  const smsPoolApiKeyValue = typeof inputSmsPoolApiKey !== 'undefined' && inputSmsPoolApiKey
    ? String(inputSmsPoolApiKey.value || '')
    : String(latestState?.smsPoolApiKey || defaultSmsPoolApiKey);
  const heroSmsApiKeyValue = phoneSmsProviderValue === phoneSmsProviderHeroSms
  ...
  const normalizePhoneSmsMinPriceValueSafe = typeof normalizePhoneSmsMinPriceValue === 'function'
    ...
    : ((value = '', provider = phoneSmsProviderValue) => {
      ...
      if (normalizedProvider === phoneSmsProviderFiveSim && typeof normalizeFiveSimMaxPriceValue === 'function') {
  ...
  const currentPhoneSmsMinPriceValue = typeof inputHeroSmsMinPrice !== 'undefined' && inputHeroSmsMinPrice
    ...
    : '';
  const heroSmsMaxPriceValue = phoneSmsProviderValue === phoneSmsProviderHeroSms
    ? currentPhoneSmsMaxPriceValue
    : normalizeHeroSmsMaxPriceValue(latestState?.heroSmsMaxPrice || '');
  const fiveSimMaxPriceValue = phoneSmsProviderValue === phoneSmsProviderFiveSim
    ? currentPhoneSmsMaxPriceValue
  ...
  const heroSmsMinPriceValue = phoneSmsProviderValue === phoneSmsProviderFiveSim
    ? normalizePhoneSmsMinPriceValueSafe(latestState?.heroSmsMinPrice || '', phoneSmsProviderHeroSms)
    : currentPhoneSmsMinPriceValue;
  const fiveSimMinPriceValue = phoneSmsProviderValue === phoneSmsProviderFiveSim
    ? currentPhoneSmsMinPriceValue
    : normalizePhoneSmsMinPriceValueSafe(latestState?.fiveSimMinPrice || '', phoneSmsProviderFiveSim);
  ...
  const smsPoolServiceValue = typeof inputSmsPoolService !== 'undefined' && inputSmsPoolService
    ? String(inputSmsPoolService.value || latestState?.smsPoolServiceLabel || defaultSmsPoolServiceLabel).trim() || defaultSmsPoolServiceLabel
    : String(latestState?.smsPoolServiceLabel || defaultSmsPoolServiceLabel);
  ...
  const selectedPhoneSmsCountry = phoneSmsProviderValue === phoneSmsProviderSmsPool
    ? ((typeof getSelectedSmsPoolCountries === 'function' ? getSelectedSmsPoolCountries()[0] : null)
      || { id: defaultSmsPoolCountryId, label: defaultSmsPoolCountryLabel })
    : phoneSmsProviderValue === phoneSmsProviderFiveSim
  ...
  const heroSmsCountry = phoneSmsProviderValue === phoneSmsProviderHeroSms
  ...
  const smsPoolCountry = phoneSmsProviderValue === phoneSmsProviderSmsPool
    ? selectedPhoneSmsCountry
    : {
      id: String(latestState?.smsPoolCountryId || defaultSmsPoolCountryId).trim().toUpperCase(),
      label: String(latestState?.smsPoolCountryLabel || defaultSmsPoolCountryLabel).trim() || defaultSmsPoolCountryLabel,
    };
  const fiveSimCountry = phoneSmsProviderValue === phoneSmsProviderFiveSim
    ? selectedPhoneSmsCountry
    : {
  ...
  const normalizedCustomEmailPoolEntries = typeof getNormalizedCustomEmailPoolEntriesState === 'function'
    ...
    : [];
  const selectedPhoneSmsCountryFallback = phoneSmsProviderValue === phoneSmsProviderSmsPool
    ? (typeof getSelectedSmsPoolCountries === 'function' ? getSelectedSmsPoolCountries() : [])
      .filter((country) => String(country.id || '').trim().toUpperCase() !== String(selectedPhoneSmsCountry.id || '').trim().toUpperCase())
  ...
  const smsPoolCountryOrderValue = typeof getSelectedSmsPoolCountries === 'function'
    ? getSelectedSmsPoolCountries()
      .map((country) => String(country.id || '').trim().toUpperCase())
      .filter(Boolean)
    : (Array.isArray(latestState?.smsPoolCountryOrder) ? latestState.smsPoolCountryOrder : [defaultSmsPoolCountryId]);
  ...
  const heroSmsCountryFallback = phoneSmsProviderValue === phoneSmsProviderHeroSms
    ? selectedPhoneSmsCountryFallback
    : normalizeHeroSmsCountryFallbackListSafe(latestState?.heroSmsCountryFallback || []);
  const smsPoolCountryFallback = phoneSmsProviderValue === phoneSmsProviderSmsPool
    ? selectedPhoneSmsCountryFallback.map((country) => ({
      id: String(country?.id || '').trim().toUpperCase(),
  ...
  const fiveSimCountryFallback = phoneSmsProviderValue === phoneSmsProviderFiveSim
    ? selectedPhoneSmsCountryFallback
  ...
  const smsBowerCountryFallback = phoneSmsProviderValue === phoneSmsProviderSmsBower
    ? selectedPhoneSmsCountryFallback
    : normalizeHeroSmsCountryFallbackListSafe(latestState?.smsBowerCountryFallback || []);
  const smsVerificationNumberCountryFallback = phoneSmsProviderValue === phoneSmsProviderSmsVerificationNumber
    ? selectedPhoneSmsCountryFallback
    : normalizeHeroSmsCountryFallbackListSafe(latestState?.smsVerificationNumberCountryFallback || []);
  const grizzlySmsCountryFallback = phoneSmsProviderValue === phoneSmsProviderGrizzlySms
    ? selectedPhoneSmsCountryFallback
    : normalizeHeroSmsCountryFallbackListSafe(latestState?.grizzlySmsCountryFallback || []);
  ...
  const plusAccountAccessStrategyOauth = typeof PLUS_ACCOUNT_ACCESS_STRATEGY_OAUTH !== 'undefined'
    ? PLUS_ACCOUNT_ACCESS_STRATEGY_OAUTH
    : 'oauth';
  ...
  const normalizePlusAccountAccessStrategySafe = typeof normalizePlusAccountAccessStrategy === 'function'
    ? normalizePlusAccountAccessStrategy
    : ((value = '') => {
      const normalized = String(value || '').trim().toLowerCase();
      return normalized === 'sub2api_codex_session' || normalized === 'cpa_codex_session'
        ? normalized
        : plusAccountAccessStrategyOauth;
    });
  const selectedExportSettings = typeof getSelectedExportSettings === 'function'
    ...
    : {
      ...
      plusAccountAccessStrategy: plusAccountAccessStrategyOauth,
    };
  const rawPanelMode = selectedExportSettings.panelMode;
  const rawPlusAccountAccessStrategy = normalizePlusAccountAccessStrategySafe(
  ...
  return {
    ...
    hotmailAliasEnabled: typeof inputHotmailAliasEnabled !== 'undefined' && inputHotmailAliasEnabled
      ...
      : false,
    outlookAliasPattern: typeof inputOutlookAliasPattern !== 'undefined' && inputOutlookAliasPattern
      ? normalizeOutlookAliasPattern(inputOutlookAliasPattern.value)
      : 'PayPal{n}',
    ...
    autoRunPreStartSkippedNodeIds: typeof getSelectedPreStartSkippedNodeIds === 'function'
      ? getSelectedPreStartSkippedNodeIds()
      : (Array.isArray(latestState?.autoRunPreStartSkippedNodeIds) ? latestState.autoRunPreStartSkippedNodeIds : []),
    ...
    smsPoolCountryFallback,
    smsPoolCountryOrder: smsPoolCountryOrderValue,
    smsPoolServiceLabel: smsPoolServiceValue,
...
function getPhoneSmsProviderLabel(provider = getSelectedPhoneSmsProvider()) {
  ...
  if (typeof window !== 'undefined' && window.PhoneSmsProviderRegistry?.getProviderLabel) {
    return window.PhoneSmsProviderRegistry.getProviderLabel(normalized);
  }
...
function normalizePhoneSmsCountryId(value, provider = getSelectedPhoneSmsProvider()) {
  ...
  if (normalizedProvider === PHONE_SMS_PROVIDER_SMSPOOL) {
    return String(value || '').trim().toUpperCase() || DEFAULT_SMS_POOL_COUNTRY_ID;
  }
...
function normalizePhoneSmsCountryLabel(value = '', provider = getSelectedPhoneSmsProvider()) {
  const normalizedProvider = normalizePhoneSmsProvider(provider);
  if (normalizedProvider === PHONE_SMS_PROVIDER_SMSPOOL) {
    return String(value || '').trim() || DEFAULT_SMS_POOL_COUNTRY_LABEL;
  }
  if (normalizedProvider === PHONE_SMS_PROVIDER_FIVE_SIM) {
...
function resolveNormalizedProviderOrderForRuntime(state = {}) {
  ...
  if (normalizedOrder.length) {
    return [fallbackProvider, ...normalizedOrder.filter((provider) => provider !== fallbackProvider)]
      .slice(0, 3);
  }
...
function getSelectedSmsPoolCountries() {
  return syncSmsPoolCountrySelectionOrderFromSelect({
    enforceMax: true,
    ensureDefault: false,
    showLimitToast: false,
  });
...
function updateHeroSmsPlatformDisplay() {
  ...
  const smsVerificationNumberProviderValue = typeof PHONE_SMS_PROVIDER_SMS_VERIFICATION_NUMBER !== 'undefined'
    ? PHONE_SMS_PROVIDER_SMS_VERIFICATION_NUMBER
    : 'sms-verification-number';
  ...
  if (provider === chatGptApiProviderValue) {
  ...
  const selected = provider === smsPoolProviderValue
    ? (getSelectedSmsPoolCountries()[0] || { id: DEFAULT_SMS_POOL_COUNTRY_ID, label: DEFAULT_SMS_POOL_COUNTRY_LABEL })
    : provider === fiveSimProviderValue
    ? (getSelectedFiveSimCountries()[0] || { id: DEFAULT_FIVE_SIM_COUNTRY_ID, label: DEFAULT_FIVE_SIM_COUNTRY_LABEL })
    : (provider === nexSmsProviderValue
  ...
  const serviceText = provider === smsPoolProviderValue
    ? String(inputSmsPoolService?.value || latestState?.smsPoolServiceLabel || DEFAULT_SMS_POOL_SERVICE_LABEL).trim() || DEFAULT_SMS_POOL_SERVICE_LABEL
    : 'OpenAI';
  const providerLabel = provider === smsPoolProviderValue
    ? 'SMSPool'
    : getPhoneSmsProviderLabel(provider);
  displayHeroSmsPlatform.textContent = `${providerLabel} / ${serviceText}${countryText}`;
  if (inputHeroSmsApiKey) {
    inputHeroSmsApiKey.placeholder = provider === smsPoolProviderValue
      ? '请输入 SMSPool API Key'
      : provider === fiveSimProviderValue
      ? '请输入 5sim API Key'
      : (provider === nexSmsProviderValue
        ...
        : (provider === smsBowerProviderValue
          ...
          : (provider === smsVerificationNumberProviderValue
            ? '请输入 SMS Verification Number API Key'
          : (provider === grizzlySmsProviderValue
            ? '请输入 GrizzlySMS API Key'
            : (provider === smsPoolProviderValue ? '请输入 SMSPool API Key' : '请输入 HeroSMS API Key')))));
  }
}
...
function buildPhoneSmsCountrySelectionFromState(state = {}, provider = getSelectedPhoneSmsProvider()) {
  ...
  if (normalizedProvider === PHONE_SMS_PROVIDER_SMSPOOL) {
    return [
      {
        id: String(state?.smsPoolCountryId || DEFAULT_SMS_POOL_COUNTRY_ID).trim().toUpperCase() || DEFAULT_SMS_POOL_COUNTRY_ID,
        label: String(state?.smsPoolCountryLabel || DEFAULT_SMS_POOL_COUNTRY_LABEL).trim() || DEFAULT_SMS_POOL_COUNTRY_LABEL,
      },
      ...(Array.isArray(state?.smsPoolCountryFallback) ? state.smsPoolCountryFallback : [])
        .map((entry) => ({
          id: String(entry?.id || entry?.countryId || entry || '').trim().toUpperCase(),
...
function restorePhoneSmsCountrySelectionFromState(state = {}, provider = getSelectedPhoneSmsProvider()) {
  ...
  if (normalized === PHONE_SMS_PROVIDER_SMSPOOL) {
    if (typeof applySmsPoolCountrySelection === 'function') {
      applySmsPoolCountrySelection(buildPhoneSmsCountrySelectionFromState(state, provider), { ensureDefault: true });
    }
    updateHeroSmsPlatformDisplay();
    return;
  }
  if (typeof applyHeroSmsFallbackSelection !== 'function') return;
...
function getSmsPoolCountryLabelByCode(code = '') {
  const normalizedCode = String(code || '').trim().toUpperCase();
  if (!normalizedCode || !selectSmsPoolCountry) {
    return '';
  }
  const matched = Array.from(selectSmsPoolCountry.options || [])
    .find((option) => String(option.value || '').trim().toUpperCase() === normalizedCode);
  return String(matched?.textContent || '').trim() || normalizedCode;
}

function renderSmsPoolCountryFallbackOrder(countries = []) {
  const display = document.getElementById('display-sms-pool-country-fallback-order');
  if (!display) {
    return;
  }
  display.textContent = '';
  ...
  countries.forEach((country, index) => {
    ...
    removeBtn.addEventListener('click', (event) => {
      event.preventDefault();
      event.stopPropagation();
      removeSmsPoolCountryFromOrder(country.id);
    });
    chip.appendChild(label);
    chip.appendChild(removeBtn);
...
function setSmsPoolCountryMenuOpen(open) {
  const nextOpen = Boolean(open);
  if (btnSmsPoolCountryMenu) {
    btnSmsPoolCountryMenu.setAttribute('aria-expanded', String(nextOpen));
  }
  if (smsPoolCountryMenu) {
    smsPoolCountryMenu.hidden = !nextOpen;
  }
}

function applySmsPoolCountryMenuFilter(keyword = '') {
  if (!smsPoolCountryMenu) {
    return;
  }
  const normalizedKeyword = String(keyword || '').trim().toLowerCase();
  const items = Array.from(smsPoolCountryMenu.querySelectorAll('.hero-sms-country-menu-item'));
  let visibleCount = 0;
  items.forEach((item) => {
    const haystack = String(item.dataset.searchText || '').toLowerCase();
    const visible = !normalizedKeyword || haystack.includes(normalizedKeyword);
  ...
  let empty = smsPoolCountryMenu.querySelector('.hero-sms-country-menu-empty');
  if (visibleCount === 0) {
    if (!empty) {
      empty = document.createElement('span');
      empty.className = 'data-value hero-sms-country-menu-empty';
      empty.textContent = '没有匹配国家';
      smsPoolCountryMenu.appendChild(empty);
    }
  } else if (empty) {
    empty.remove();
  }
...
function updateSmsPoolCountryMenuSummary(selectedCountries = []) {
  if (!btnSmsPoolCountryMenu) {
    return;
  }
  if (!selectedCountries.length) {
    btnSmsPoolCountryMenu.textContent = `未选择 (0/${HERO_SMS_COUNTRY_SELECTION_MAX})`;
    return;
  }
  btnSmsPoolCountryMenu.textContent = `${selectedCountries.map((country) => country.label).join(' / ')} (${selectedCountries.length}/${HERO_SMS_COUNTRY_SELECTION_MAX})`;
}

function renderSmsPoolCountryChoiceButtons() {
  if (!smsPoolCountryMenu || !selectSmsPoolCountry) {
    return;
  }
  const options = Array.from(selectSmsPoolCountry.options || []);
  const selectedOrder = [...smsPoolCountrySelectionOrder];
  const selectedSet = new Set(selectedOrder);
  smsPoolCountryMenu.innerHTML = '';
  const searchWrap = document.createElement('div');
  searchWrap.className = 'hero-sms-country-menu-search';
  const searchInput = document.createElement('input');
  searchInput.type = 'search';
  searchInput.className = 'data-input mono hero-sms-country-menu-search-input';
  searchInput.placeholder = '搜索国家（中/英/代码）';
  searchInput.value = smsPoolCountryMenuSearchKeyword;
  searchInput.addEventListener('input', () => {
    smsPoolCountryMenuSearchKeyword = String(searchInput.value || '').trim();
    applySmsPoolCountryMenuFilter(smsPoolCountryMenuSearchKeyword);
  });
  searchWrap.appendChild(searchInput);
  smsPoolCountryMenu.appendChild(searchWrap);
  if (!options.length) {
    const empty = document.createElement('span');
    empty.className = 'data-value hero-sms-country-menu-empty';
    empty.textContent = '暂无国家选项';
    smsPoolCountryMenu.appendChild(empty);
    updateSmsPoolCountryMenuSummary([]);
    return;
  }
  options.forEach((option) => {
    const countryId = String(option.value || '').trim().toUpperCase();
    if (!countryId) {
      return;
    }
    const item = document.createElement('button');
    ...
    item.className = 'hero-sms-country-menu-item';
    item.dataset.countryId = countryId;
    item.dataset.searchText = smsPoolCountrySearchTextById.get(countryId) || `${countryId} ${option.textContent || ''}`;
    const selected = selectedSet.has(countryId);
    item.setAttribute('aria-pressed', selected ? 'true' : 'false');
    item.textContent = option.textContent || countryId;
    item.addEventListener('click', (event) => {
      event.preventDefault();
      toggleSmsPoolCountryInOrder(countryId);
    });
    smsPoolCountryMenu.appendChild(item);
  });
  applySmsPoolCountryMenuFilter(smsPoolCountryMenuSearchKeyword);
}

function syncSmsPoolCountrySelectionOrderFromSelect(options = {}) {
  const selectionLimit = Math.max(1, Math.floor(Number(options.maxSelection) || HERO_SMS_COUNTRY_SELECTION_MAX));
  const ensureDefault = options.ensureDefault !== false;
  const enforceMax = options.enforceMax !== false;
  const showLimitToast = Boolean(options.showLimitToast);
  if (!selectSmsPoolCountry) {
    smsPoolCountrySelectionOrder = [];
    renderSmsPoolCountryFallbackOrder([]);
    updateSmsPoolCountryMenuSummary([]);
    return [];
  }
  const selectedIds = Array.from(selectSmsPoolCountry.options || [])
    .filter((option) => option.selected)
    .map((option) => String(option.value || '').trim().toUpperCase())
    .filter(Boolean);
  const selectedSet = new Set(selectedIds);
  let nextOrder = smsPoolCountrySelectionOrder.filter((id) => selectedSet.has(id));
  selectedIds.forEach((id) => {
    if (!nextOrder.includes(id)) {
      nextOrder.push(id);
    }
  ...
  if (ensureDefault && !nextOrder.length) {
    nextOrder = [DEFAULT_SMS_POOL_COUNTRY_ID];
  }
  if (enforceMax && nextOrder.length > selectionLimit) {
    const droppedCount = nextOrder.length - selectionLimit;
    nextOrder = nextOrder.slice(0, selectionLimit);
    if (showLimitToast && droppedCount > 0 && typeof showToast === 'function') {
      showToast(`SMSPool 国家最多选择 ${selectionLimit} 个，已保留前 ${selectionLimit} 个。`, 'warn', 2200);
    }
  }
  const nextOrderSet = new Set(nextOrder);
  Array.from(selectSmsPoolCountry.options || []).forEach((option) => {
    option.selected = nextOrderSet.has(String(option.value || '').trim().toUpperCase());
  });
  smsPoolCountrySelectionOrder = [...nextOrder];
  const selectedCountries = smsPoolCountrySelectionOrder.map((id) => ({
    id,
    label: getSmsPoolCountryLabelByCode(id),
  }));
  renderSmsPoolCountryFallbackOrder(selectedCountries);
  updateSmsPoolCountryMenuSummary(selectedCountries);
  renderSmsPoolCountryChoiceButtons();
  return selectedCountries;
}

function applySmsPoolCountrySelection(countries = [], options = {}) {
  const normalized = (Array.isArray(countries) ? countries : [])
    .map((entry) => String(entry?.id || entry).trim().toUpperCase())
    .filter(Boolean)
    .slice(0, HERO_SMS_COUNTRY_SELECTION_MAX);
  smsPoolCountrySelectionOrder = [...normalized];
  if (selectSmsPoolCountry) {
...
function toggleSmsPoolCountryInOrder(id = '') {
  const normalizedId = String(id || '').trim().toUpperCase();
  if (!normalizedId || !selectSmsPoolCountry) {
    return [];
  }
  const option = Array.from(selectSmsPoolCountry.options || [])
    .find((entry) => String(entry.value || '').trim().toUpperCase() === normalizedId);
  if (option) {
    option.selected = !option.selected;
  }
  const nextOrder = syncSmsPoolCountrySelectionOrderFromSelect({
    enforceMax: true,
    ensureDefault: false,
    showLimitToast: true,
  });
...
function removeSmsPoolCountryFromOrder(id = '') {
  const normalizedId = String(id || '').trim().toUpperCase();
  smsPoolCountrySelectionOrder = smsPoolCountrySelectionOrder.filter((entry) => entry !== normalizedId);
  if (selectSmsPoolCountry) {
    Array.from(selectSmsPoolCountry.options || []).forEach((option) => {
      if (String(option.value || '').trim().toUpperCase() === normalizedId) {
        option.selected = false;
      }
    });
  }
...
async function loadSmsPoolCountries() {
  if (!selectSmsPoolCountry) {
    return;
  }
  const previousOrder = [...smsPoolCountrySelectionOrder];
  const applyOptions = (items = []) => {
    selectSmsPoolCountry.innerHTML = '';
    smsPoolCountrySearchTextById.clear();
    items.forEach((entry) => {
      ...
      selectSmsPoolCountry.appendChild(option);
      smsPoolCountrySearchTextById.set(id, entry.searchText || `${option.textContent} ${id}`);
    });
  };
  const currentApiKey = String(inputSmsPoolApiKey?.value || latestState?.smsPoolApiKey || DEFAULT_SMS_POOL_API_KEY || '').trim();
  const currentService = String(latestState?.smsPoolServiceId || DEFAULT_SMS_POOL_SERVICE_ID || '671').trim() || '671';
  const fallbackItems = [{
    id: DEFAULT_SMS_POOL_COUNTRY_ID,
    label: DEFAULT_SMS_POOL_COUNTRY_LABEL,
    searchText: `${DEFAULT_SMS_POOL_COUNTRY_LABEL} ${DEFAULT_SMS_POOL_COUNTRY_ID} 1`,
  }];
  try {
    const response = currentApiKey
      ? await fetch('https://api.smspool.net/request/suggested_countries', {
        method: 'POST',
        cache: 'no-store',
        headers: {
      ...
      : await fetch('https://api.smspool.net/country/retrieve_all', {
        cache: 'no-store',
        headers: { Accept: 'application/json' },
      });
    if (!response.ok) {
    ...
    const items = Array.isArray(payload)
      ? payload.map((entry) => ({
        id: String(entry?.short_name || entry?.ID || entry?.country_id || '').trim().toUpperCase(),
        label: String(entry?.name || entry?.short_name || '').trim(),
        price: entry?.price !== undefined && entry?.price !== null ? String(entry.price).trim() : '',
        pool: entry?.pool !== undefined && entry?.pool !== null ? String(entry.pool).trim() : '',
    ...
    items.forEach((entry) => {
      ...
      if (details && !String(entry.label || '').includes(details)) {
        entry.label = `${entry.label} ${details}`;
      }
    });
    applyOptions(items.length ? items : fallbackItems);
  } catch (error) {
  ...
  applySmsPoolCountrySelection(previousOrder.length ? previousOrder : (Array.isArray(latestState?.smsPoolCountryOrder) ? latestState.smsPoolCountryOrder : [DEFAULT_SMS_POOL_COUNTRY_ID]), {
    ensureDefault: true,
  });
}
...
async function previewHeroSmsPriceTiers() {
  ...
  for (const provider of providerOrder) {
    ...
    if (provider === smsPoolProviderValue) {
      const selectedCountries = syncSmsPoolCountrySelectionOrderFromSelect({
        enforceMax: true,
        ensureDefault: false,
        showLimitToast: false,
      });
      ...
      const apiKey = normalizeProvider(activeProvider) === smsPoolProviderValue
        ? String(inputSmsPoolApiKey?.value || '').trim()
      ...
      for (const country of candidates) {
        const countryLabel = String(country.label || getSmsPoolCountryLabelByCode(country.id) || country.id).trim();
        try {
          const response = await fetch('https://api.smspool.net/request/suggested_countries', {
            method: 'POST',
            cache: 'no-store',
            headers: {
              Accept: 'application/json',
            ...
            body: new URLSearchParams({
              key: apiKey,
              service: String(latestState?.smsPoolServiceId || latestState?.smsPoolServiceCode || '671').trim() || '671',
            }).toString(),
          });
          const payload = await response.json().catch(() => []);
          if (!response.ok) {
          ...
          const matchedRecords = records.filter((entry) => {
            const shortName = String(entry?.short_name || '').trim().toUpperCase();
            const numericId = String(entry?.country_id || entry?.ID || '').trim().toUpperCase();
            return shortName === country.id || numericId === country.id;
          });
          const tierEntries = matchedRecords
            .map((entry) => {
              const numericPrice = Number(String(entry?.price || '').replace(/[$,\s]/g, ''));
              if (!Number.isFinite(numericPrice) || numericPrice <= 0) {
                return null;
              }
              return {
                price: Math.round(numericPrice * 10000) / 10000,
                count: null,
                pool: String(entry?.pool || '').trim(),
              };
            })
            .filter(Boolean);
          const uniqueSorted = Array.from(new Set(tierEntries.map((entry) => entry.price))).sort((left, right) => left - right);
          const rangePrices = filterPhoneSmsPriceValuesForPreviewRange(uniqueSorted, priceRange);
          const filteredTierEntries = filterPhoneSmsPriceEntriesForPreviewRange(tierEntries, priceRange);
          if (!rangePrices.length) {
          ...
          const lowest = rangePrices[0];
          const tierText = formatPriceTiersForPreview(filteredTierEntries, { maxPrice: priceRange.maxPrice });
          const pools = Array.from(new Set(matchedRecords.map((entry) => String(entry?.pool || '').trim()).filter(Boolean)));
          const lowestLabel = priceRange.hasMinPrice || priceRange.hasMaxPrice ? '区间内最低' : '最低';
          smsPoolLines.push(`${countryLabel}: ${lowestLabel} ${lowest}${pools.length ? `；可用池：${pools.join('/')}` : ''}${tierText ? `；档位：${tierText}` : ''}`);
        } catch (error) {
...
async function previewPhoneSmsBalance() {
  ...
  const apiKey = provider === PHONE_SMS_PROVIDER_SMSPOOL
    ? String(inputSmsPoolApiKey?.value || '').trim()
  ...
  try {
    ...
    if (provider === PHONE_SMS_PROVIDER_SMSPOOL) {
      url = new URL('https://api.smspool.net/request/balance');
    ...
    if (provider === PHONE_SMS_PROVIDER_SMSPOOL) {
      ...
      requestOptions.body = new URLSearchParams({ key: apiKey }).toString();
    } else if (provider === PHONE_SMS_PROVIDER_FIVE_SIM) {
      requestOptions.headers = {
        Accept: 'application/json',
    ...
    if (provider === PHONE_SMS_PROVIDER_SMSPOOL) {
      ...
      displayPhoneSmsBalance.textContent = Number.isFinite(balance)
        ...
        : `SMSPool 余额：${describeHeroSmsPreviewPayload(payload) || '未知'}`;
    } else if (provider === PHONE_SMS_PROVIDER_FIVE_SIM) {
      const balance = Number(payload?.balance);
...
function updateSignupMethodUI(options = {}) {
  ...
  const currentAccountAccessStrategy = typeof currentPlusAccountAccessStrategy !== 'undefined'
    ? currentPlusAccountAccessStrategy
    : (typeof normalizePlusAccountAccessStrategy === 'function'
      ? normalizePlusAccountAccessStrategy(latestState?.plusAccountAccessStrategy || 'oauth')
      : String(latestState?.plusAccountAccessStrategy || 'oauth').trim().toLowerCase());
  const smsOauthStrategyValue = typeof PLUS_ACCOUNT_ACCESS_STRATEGY_SMS_OAUTH !== 'undefined'
    ? PLUS_ACCOUNT_ACCESS_STRATEGY_SMS_OAUTH
    : 'sms_oauth';
  const phoneBindOauthStrategyValue = typeof PLUS_ACCOUNT_ACCESS_STRATEGY_PHONE_BIND_OAUTH !== 'undefined'
    ? PLUS_ACCOUNT_ACCESS_STRATEGY_PHONE_BIND_OAUTH
    : 'phone_bind_oauth';
  const smsOauthStrategyActive = currentAccountAccessStrategy === smsOauthStrategyValue;
  const phoneBindOauthStrategyActive = currentAccountAccessStrategy === phoneBindOauthStrategyValue;
...
function updatePhoneVerificationSettingsUI() {
  ...
  const showProviderConfig = canShowPhoneSettings;
  ...
  const currentAccountAccessStrategyForPhoneSettings = typeof currentPlusAccountAccessStrategy !== 'undefined'
    ? currentPlusAccountAccessStrategy
    : (typeof normalizePlusAccountAccessStrategy === 'function'
      ? normalizePlusAccountAccessStrategy(latestState?.plusAccountAccessStrategy || 'oauth')
      : String(latestState?.plusAccountAccessStrategy || 'oauth').trim().toLowerCase());
  const smsOauthStrategyForPhoneSettings = typeof PLUS_ACCOUNT_ACCESS_STRATEGY_SMS_OAUTH !== 'undefined'
    ? PLUS_ACCOUNT_ACCESS_STRATEGY_SMS_OAUTH
    : 'sms_oauth';
  const smsOauthStrategyActive = currentAccountAccessStrategyForPhoneSettings === smsOauthStrategyForPhoneSettings;
  ...
  const heroLikeProvider = heroProvider || smsBowerProvider || smsVerificationNumberProvider || grizzlySmsProvider;
  const priceCapableProvider = heroLikeProvider || fiveSimProvider || nexSmsProvider || smsPoolProvider;
  const showSmsPoolConfig = showProviderConfig;
  if (phoneVerificationSection) {
    phoneVerificationSection.style.display = canShowPhoneSettings ? '' : 'none';
  }
  ...
  if (rowPhoneVerificationFold) {
    rowPhoneVerificationFold.style.display = showProviderConfig ? '' : 'none';
  }
  ...
  const phoneVerificationRows = [
    ...
    typeof rowSmsPoolApiKey !== 'undefined' ? rowSmsPoolApiKey : null,
    typeof rowSmsPoolCountry !== 'undefined' ? rowSmsPoolCountry : null,
  ...
  if (typeof rowPhoneSmsProvider !== 'undefined' && rowPhoneSmsProvider) rowPhoneSmsProvider.style.display = showProviderConfig ? '' : 'none';
  if (typeof rowPhoneSmsProviderOrder !== 'undefined' && rowPhoneSmsProviderOrder) rowPhoneSmsProviderOrder.style.display = showProviderConfig ? '' : 'none';
  if (typeof rowPhoneSmsProviderOrderActions !== 'undefined' && rowPhoneSmsProviderOrderActions) rowPhoneSmsProviderOrderActions.style.display = showProviderConfig ? '' : 'none';
  if (typeof rowSmsPoolApiKey !== 'undefined' && rowSmsPoolApiKey) rowSmsPoolApiKey.style.display = showSmsPoolConfig ? '' : 'none';
  if (typeof rowSmsPoolCountry !== 'undefined' && rowSmsPoolCountry) rowSmsPoolCountry.style.display = showSmsPoolConfig ? '' : 'none';
  ...
  if (typeof btnSmsPoolCountryMenu !== 'undefined' && btnSmsPoolCountryMenu) {
    btnSmsPoolCountryMenu.disabled = !showSmsPoolConfig;
  }
  if (typeof btnSmsPoolCountryClear !== 'undefined' && btnSmsPoolCountryClear) {
    btnSmsPoolCountryClear.disabled = !showSmsPoolConfig;
  }
...
function updatePlusModeUI() {
  ...
  if (typeof inputPlusModeEnabled !== 'undefined' && inputPlusModeEnabled) {
    const locked = (typeof isAutoRunLockedPhase === 'function' && isAutoRunLockedPhase())
      || (typeof isAutoRunScheduledPhase === 'function' && isAutoRunScheduledPhase())
      || (typeof isAutoRunPausedPhase === 'function' && isAutoRunPausedPhase());
    inputPlusModeEnabled.disabled = locked || !supportsPlusMode;
    inputPlusModeEnabled.title = supportsPlusMode
      ? '开启后使用 Plus Checkout 支付授权流程'
      : '当前 flow 不支持 Plus 模式';
  }
  if (typeof plusModeCaption !== 'undefined' && plusModeCaption) {
    plusModeCaption.textContent = enabled
      ? '开启后使用 Plus Checkout 支付授权流程'
      : '关闭后跳过 Plus Checkout 支付授权流程';
  }
...
async function saveSettings(options = {}) {
  ...
  try {
    const sendSettingsMessage = typeof sendRuntimeMessageWithTimeout === 'function'
...
function normalizePreStartSkippedNodeIds(value = [], validNodeIds = NODE_IDS) {
  const validSet = new Set((Array.isArray(validNodeIds) ? validNodeIds : [])
...
function getSelectedPreStartSkippedNodeIds() {
  return normalizePreStartSkippedNodeIds(Array.from(preStartSkippedNodeIds), NODE_IDS);
}

function setPreStartSkippedNodeIds(nodeIds = [], options = {}) {
  const normalized = normalizePreStartSkippedNodeIds(nodeIds, NODE_IDS);
  preStartSkippedNodeIds = new Set(normalized);
  if (options.syncState !== false) {
    syncLatestState({ autoRunPreStartSkippedNodeIds: normalized });
  }
  renderPreStartSkipOptions();
  updateButtonStates();
  return normalized;
}

function renderPreStartSkipOptions() {
  if (!preStartSkipOptions) {
    return;
  }
  preStartSkipOptions.textContent = '';
  const selected = getSelectedPreStartSkippedNodeIds();
  const selectedSet = new Set(selected);
  const locked = isAutoRunLockedPhase() || isAutoRunScheduledPhase() || isAutoRunPausedPhase();
  if (preStartSkipControl) {
    preStartSkipControl.style.display = workflowNodes.length ? '' : 'none';
  }
  if (preStartSkipSummary) {
    preStartSkipSummary.textContent = selected.length
      ? `启动前跳过：${selected.length} 项`
      : '启动前跳过：无';
    preStartSkipSummary.title = selected.length ? selected.join(' / ') : '启动前跳过：无';
  }
  workflowNodes.forEach((node) => {
    ...
    button.type = 'button';
    button.className = `prestart-skip-chip${selectedSet.has(nodeId) ? ' active' : ''}`;
    button.dataset.nodeId = nodeId;
    button.setAttribute('aria-pressed', selectedSet.has(nodeId) ? 'true' : 'false');
    button.title = `自动开始前标记节点 ${nodeId} 为已跳过`;
    button.textContent = `${step || node.displayOrder || ''}. ${node.title || nodeId}`;
    button.disabled = locked;
    button.addEventListener('click', () => {
      const next = new Set(getSelectedPreStartSkippedNodeIds());
      if (next.has(nodeId)) {
        next.delete(nodeId);
      } else {
        next.add(nodeId);
      }
      setPreStartSkippedNodeIds(Array.from(next));
      markSettingsDirty(true);
      saveSettings({ silent: true }).catch(() => {});
    });
    preStartSkipOptions.appendChild(button);
  });
}

function updatePreStartSkipControlState() {
  const locked = isAutoRunLockedPhase() || isAutoRunScheduledPhase() || isAutoRunPausedPhase();
  preStartSkipOptions?.querySelectorAll?.('.prestart-skip-chip')?.forEach((button) => {
    button.disabled = locked;
  });
}
...
function renderStepsList() {
  ...
  setPreStartSkippedNodeIds(getSelectedPreStartSkippedNodeIds(), { syncState: false });
  renderStepStatuses();
  updateButtonStates();
}
...
function applySettingsState(state) {
  ...
  const resolveIpProxyService = (value) => (typeof normalizeIpProxyService === 'function'
    ...
    : (['711proxy', 'clash'].includes(String(value || '').trim().toLowerCase())
      ? String(value || '').trim().toLowerCase()
      : fallbackIpProxyService));
  ...
  if (inputIcloudAliasLabelPattern) {
    inputIcloudAliasLabelPattern.value = normalizeIcloudAliasLabelPattern(state?.icloudAliasLabelPattern);
  }
  ...
  if (inputGmailAliasPattern) {
    inputGmailAliasPattern.value = String(state?.gmailAliasPattern || '').trim();
  }
  if (inputGmailMailboxUrl) {
    inputGmailMailboxUrl.value = String(state?.gmailMailboxUrl || '').trim();
  }
  ...
  if (typeof inputOutlookAliasPattern !== 'undefined' && inputOutlookAliasPattern) {
    inputOutlookAliasPattern.value = normalizeOutlookAliasPattern(state?.outlookAliasPattern);
  }
  ...
  setPreStartSkippedNodeIds(state?.autoRunPreStartSkippedNodeIds || [], { syncState: false });
  ...
  if (typeof inputSmsPoolApiKey !== 'undefined' && inputSmsPoolApiKey) {
    inputSmsPoolApiKey.value = String(state?.smsPoolApiKey || DEFAULT_SMS_POOL_API_KEY);
  }
  if (typeof inputSmsPoolService !== 'undefined' && inputSmsPoolService) {
    inputSmsPoolService.value = String(state?.smsPoolServiceLabel || DEFAULT_SMS_POOL_SERVICE_LABEL);
  }
...
async function restoreState() {
  try {
    if (!hasChromeRuntimeApi()) {
      const state = buildLocalSidepanelPreviewState();
...
function renderUpdateReleaseList(releases = []) {
  ...
  releases.forEach((release) => {
    ...
    version.textContent = release.displayVersion || `YanAutoPlus ${release.version}`;
...
function renderReleaseSnapshot(snapshot) {
  ...
  switch (snapshot?.status) {
    ...
    case 'ignored': {
      extensionUpdateStatus.textContent = localVersionText || 'YanAutoPlus 0.0';
...
async function initializeReleaseInfo() {
  ...
  const manifest = getChromeRuntimeManifest();
  const localVersion = sidepanelUpdateService?.getLocalVersionLabel?.(manifest)
    || manifest?.version_name
    || (manifest?.version ? `YanAutoPlus ${manifest.version}` : '');
  extensionUpdateStatus.textContent = localVersion || 'YanAutoPlus 0.0';
  ...
  const snapshot = hasChromeRuntimeApi()
    ? await sidepanelUpdateService.getReleaseSnapshot()
    : {
      status: 'empty',
      localVersion: localVersion || 'YanAutoPlus local preview',
      newerReleases: [],
      logUrl: fallbackReleaseUrl,
      releasesPageUrl: fallbackReleaseUrl,
    };
...
function updateMailProviderUI() {
  ...
  if (typeof rowIcloudAliasLabelPattern !== 'undefined' && rowIcloudAliasLabelPattern) {
    rowIcloudAliasLabelPattern.style.display = ((useEmailGenerator && useIcloud) || useIcloudProvider) ? '' : 'none';
  }
  ...
  if (rowGmailAliasPattern) {
    rowGmailAliasPattern.style.display = useGmail && useGeneratedAlias ? '' : 'none';
  }
  if (rowGmailMailboxUrl) {
    rowGmailMailboxUrl.style.display = useGmail ? '' : 'none';
  }
  ...
  if (typeof rowOutlookAliasPattern !== 'undefined' && rowOutlookAliasPattern) {
    rowOutlookAliasPattern.style.display = useHotmail && hotmailAliasEnabled ? '' : 'none';
  }
...
function updateButtonStates() {
  ...
  updatePreStartSkipControlState();
...
async function startAutoRunFromCurrentSettings() {
  ...
  const hostedCheckoutValidation = typeof validateHostedCheckoutContactConfig === 'function'
    ? validateHostedCheckoutContactConfig({ focusOnError: true })
    : { valid: true, message: '' };
  if (!hostedCheckoutValidation.valid) {
    clearPendingAutoRunStartRunCount();
    throw new Error(hostedCheckoutValidation.message || 'Hosted checkout 配置不完整。');
  }
  const cloudCheckoutValidation = typeof validatePlusCheckoutCloudConversionConfig === 'function'
    ? validatePlusCheckoutCloudConversionConfig({ focusOnError: true })
    : { valid: true, message: '' };
  if (!cloudCheckoutValidation.valid) {
    clearPendingAutoRunStartRunCount();
    throw new Error(cloudCheckoutValidation.message || '云端支付转换配置不完整。');
  }
  if (typeof ensureGpcApiKeyReadyForStart === 'function' && !(await ensureGpcApiKeyReadyForStart())) {
    clearPendingAutoRunStartRunCount();
    return false;
  }
  ...
  const autoRunRetryNonFreeTrial = typeof inputAutoRunRetryNonFreeTrial !== 'undefined'
    ? Boolean(inputAutoRunRetryNonFreeTrial?.checked)
    : Boolean(latestState?.autoRunRetryNonFreeTrial);
  const autoRunRetryPaypalCallback = typeof inputAutoRunRetryPaypalCallback !== 'undefined'
    ? Boolean(inputAutoRunRetryPaypalCallback?.checked)
    : Boolean(latestState?.autoRunRetryPaypalCallback);
  const autoRunPreStartSkippedNodeIds = typeof getSelectedPreStartSkippedNodeIds === 'function'
    ? getSelectedPreStartSkippedNodeIds()
    : (Array.isArray(latestState?.autoRunPreStartSkippedNodeIds) ? latestState.autoRunPreStartSkippedNodeIds : []);
...
inputGmailAliasPattern?.addEventListener('input', () => {
  syncLatestState({ gmailAliasPattern: inputGmailAliasPattern.value.trim() });
  markSettingsDirty(true);
  scheduleSettingsAutoSave();
});
inputGmailAliasPattern?.addEventListener('blur', () => {
  syncLatestState({ gmailAliasPattern: inputGmailAliasPattern.value.trim() });
  saveSettings({ silent: true }).catch(() => { });
});
inputGmailMailboxUrl?.addEventListener('input', () => {
  syncLatestState({ gmailMailboxUrl: inputGmailMailboxUrl.value.trim() });
  markSettingsDirty(true);
  scheduleSettingsAutoSave();
});
inputGmailMailboxUrl?.addEventListener('blur', () => {
  syncLatestState({ gmailMailboxUrl: inputGmailMailboxUrl.value.trim() });
  saveSettings({ silent: true }).catch(() => { });
});
...
inputSmsPoolApiKey?.addEventListener('input', () => {
  markSettingsDirty(true);
  scheduleSettingsAutoSave();
});
inputSmsPoolApiKey?.addEventListener('blur', () => {
  if (getSelectedPhoneSmsProvider() === PHONE_SMS_PROVIDER_SMSPOOL) {
    loadSmsPoolCountries().catch(() => { });
  }
  saveSettings({ silent: true }).catch(() => { });
});
inputSmsPoolService?.addEventListener('input', () => {
  updateHeroSmsPlatformDisplay();
  markSettingsDirty(true);
  scheduleSettingsAutoSave();
});
inputSmsPoolService?.addEventListener('blur', () => {
  updateHeroSmsPlatformDisplay();
  if (getSelectedPhoneSmsProvider() === PHONE_SMS_PROVIDER_SMSPOOL) {
    loadSmsPoolCountries().catch(() => { });
  }
...
inputSmsPoolPool?.addEventListener('input', () => {
  markSettingsDirty(true);
  scheduleSettingsAutoSave();
});
...
btnSmsPoolCountryClear?.addEventListener('click', (event) => {
  event.preventDefault();
  event.stopPropagation();
  smsPoolCountrySelectionOrder = [];
  if (selectSmsPoolCountry) {
  ...
  syncSmsPoolCountrySelectionOrderFromSelect({
    enforceMax: true,
    ensureDefault: true,
    showLimitToast: false,
  });
...
function getPhoneSmsCountrySelectionForProvider(provider = getSelectedPhoneSmsProvider(), options = {}) {
  const normalizedProvider = normalizePhoneSmsProvider(provider);
  const countrySelect = normalizedProvider === PHONE_SMS_PROVIDER_SMSPOOL
    ? selectSmsPoolCountry
    : (selectHeroSmsCountry || selectHeroSmsCountryFallback);
  const selectionLimit = Math.max(1, Math.floor(Number(options.maxSelection) || HERO_SMS_COUNTRY_SELECTION_MAX));
  const ensureDefault = options.ensureDefault !== false;
  const defaultCountry = normalizedProvider === PHONE_SMS_PROVIDER_SMSPOOL
  ...
  const selectedSet = new Set(selectedIds.map((id) => String(id)));
  let orderedIds = (normalizedProvider === PHONE_SMS_PROVIDER_SMSPOOL ? smsPoolCountrySelectionOrder : heroSmsCountrySelectionOrder)
...
async function switchPhoneSmsProvider(nextProvider) {
  var PHONE_SMS_PROVIDER_SMSPOOL = 'smspool';
  var PHONE_SMS_PROVIDER_SMSBOWER = 'smsbower';
  var PHONE_SMS_PROVIDER_SMS_VERIFICATION_NUMBER = 'sms-verification-number';
  var PHONE_SMS_PROVIDER_GRIZZLYSMS = 'grizzlysms';
  var PHONE_SMS_PROVIDER_CHATGPT_API = 'chatgpt-api';
  const smsPoolProviderValue = typeof PHONE_SMS_PROVIDER_SMSPOOL !== 'undefined'
    ? PHONE_SMS_PROVIDER_SMSPOOL
    : 'smspool';
  ...
  if (previousProvider === smsPoolProviderValue) {
    patch.smsPoolApiKey = String(inputSmsPoolApiKey?.value || latestState?.smsPoolApiKey || '');
    patch.smsPoolCountryId = String(currentPrimary?.id || DEFAULT_SMS_POOL_COUNTRY_ID).trim().toUpperCase();
    patch.smsPoolCountryLabel = String(currentPrimary?.label || DEFAULT_SMS_POOL_COUNTRY_LABEL).trim() || DEFAULT_SMS_POOL_COUNTRY_LABEL;
    patch.smsPoolCountryFallback = currentFallback.map((country) => ({
      id: String(country?.id || '').trim().toUpperCase(),
      label: String(country?.label || '').trim(),
    })).filter((country) => country.id);
    patch.smsPoolCountryOrder = [currentPrimary, ...currentFallback]
      .map((country) => String(country?.id || '').trim().toUpperCase())
      .filter(Boolean);
  ...
  if (typeof inputSmsPoolApiKey !== 'undefined' && inputSmsPoolApiKey) {
    inputSmsPoolApiKey.value = String(latestState?.smsPoolApiKey || DEFAULT_SMS_POOL_API_KEY);
  }
  if (typeof inputSmsPoolService !== 'undefined' && inputSmsPoolService) {
    inputSmsPoolService.value = String(latestState?.smsPoolServiceLabel || DEFAULT_SMS_POOL_SERVICE_LABEL);
  }
  if (typeof inputSmsPoolPool !== 'undefined' && inputSmsPoolPool) {
  ...
  if (normalizedNextProvider === smsPoolProviderValue) {
  ...
  if (normalizedNextProvider === smsPoolProviderValue) {
    restoredPrimary = {
      id: String(latestState?.smsPoolCountryId || DEFAULT_SMS_POOL_COUNTRY_ID).trim().toUpperCase(),
      label: String(latestState?.smsPoolCountryLabel || DEFAULT_SMS_POOL_COUNTRY_LABEL).trim() || DEFAULT_SMS_POOL_COUNTRY_LABEL,
    };
    restoredFallback = Array.isArray(latestState?.smsPoolCountryFallback) ? latestState.smsPoolCountryFallback : [];
  ...
  if (normalizedNextProvider === smsPoolProviderValue) {
    applySmsPoolCountrySelection([restoredPrimary, ...restoredFallback], { ensureDefault: true });
  } else {
...
selectPhoneSmsProvider?.addEventListener('change', async () => {
  ...
  } else if (selectPhoneSmsProvider?.value === PHONE_SMS_PROVIDER_SMSPOOL) {
    await loadSmsPoolCountries().catch(() => { });
    applySmsPoolCountrySelection(
      Array.isArray(latestState?.smsPoolCountryOrder) ? latestState.smsPoolCountryOrder : [DEFAULT_SMS_POOL_COUNTRY_ID],
      { ensureDefault: true }
    );
...
inputOutlookAliasPattern?.addEventListener('input', () => {
  syncLatestState({ outlookAliasPattern: normalizeOutlookAliasPattern(inputOutlookAliasPattern.value) });
  markSettingsDirty(true);
  scheduleSettingsAutoSave();
});
inputOutlookAliasPattern?.addEventListener('blur', () => {
  inputOutlookAliasPattern.value = normalizeOutlookAliasPattern(inputOutlookAliasPattern.value);
  syncLatestState({ outlookAliasPattern: inputOutlookAliasPattern.value });
  saveSettings({ silent: true }).catch(() => { });
});
  ...
  switch (message.type) {
    ...
    case 'DATA_UPDATED': {
      ...
      if (
        ...
        || message.payload.ipProxyAppliedClashNode !== undefined
      ...
      if (message.payload.gmailAliasPattern !== undefined && inputGmailAliasPattern) {
        inputGmailAliasPattern.value = String(message.payload.gmailAliasPattern || '').trim();
      }
      if (message.payload.gmailMailboxUrl !== undefined && inputGmailMailboxUrl) {
        inputGmailMailboxUrl.value = String(message.payload.gmailMailboxUrl || '').trim();
      }
      if (message.payload.gmailBaseEmail !== undefined && selectMailProvider.value === GMAIL_PROVIDER) {
        setManagedAliasBaseEmailInputForProvider(GMAIL_PROVIDER, latestState);
      }
      ...
      if (message.payload.icloudAliasLabelPattern !== undefined && inputIcloudAliasLabelPattern) {
        inputIcloudAliasLabelPattern.value = normalizeIcloudAliasLabelPattern(message.payload.icloudAliasLabelPattern);
      }
      ...
      if (message.payload.autoRunPreStartSkippedNodeIds !== undefined) {
        setPreStartSkippedNodeIds(message.payload.autoRunPreStartSkippedNodeIds || [], { syncState: false });
      }
      ...
      if (message.payload.outlookAliasPattern !== undefined && inputOutlookAliasPattern) {
        inputOutlookAliasPattern.value = normalizeOutlookAliasPattern(message.payload.outlookAliasPattern);
      }
...
document.addEventListener('click', (event) => {
  ...
  const clickedInsideSmsPoolCountryMenu = Boolean(smsPoolCountryMenuShell?.contains(event.target));
  ...
  const smsPoolCountryMenuOpen = btnSmsPoolCountryMenu?.getAttribute('aria-expanded') === 'true';
  if (smsPoolCountryMenuOpen && !clickedInsideSmsPoolCountryMenu) {
    setSmsPoolCountryMenuOpen(false);
  }
...
document.addEventListener('keydown', (event) => {
  ...
  if (btnSmsPoolCountryMenu?.getAttribute('aria-expanded') === 'true') {
    setSmsPoolCountryMenuOpen(false);
  }
...
Promise.allSettled([
  loadSmsPoolCountries(),
  loadHeroSmsCountries(),
  loadFiveSimCountries(),
  loadNexSmsCountries({ silent: true }),
]).then((results) => {
  const smsPoolResult = results[0];
  const heroResult = results[1];
  const fiveSimResult = results[2];
  const nexSmsResult = results[3];
  if (smsPoolResult?.status === 'rejected') {
    console.error('加载 SMSPool 国家列表失败：', smsPoolResult.reason);
  }
