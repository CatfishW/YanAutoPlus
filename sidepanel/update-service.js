(() => {
  const GITHUB_OWNER = 'CatfishW';
  const GITHUB_REPO = 'YanAutoPlus';
  ...
  const VERSION_FAMILY_YANAUTOPLUS = 'yanautoplus';
  ...
  function getVersionFamily(version, fallbackFamily = VERSION_FAMILY_LEGACY) {
    ...
    if (/^(?:yanautoplus|flowpilot)/i.test(trimmed)) {
      return VERSION_FAMILY_YANAUTOPLUS;
    }
  ...
  function getVersionFamilyPrefix(family) {
    if (family === VERSION_FAMILY_YANAUTOPLUS) {
      return 'YanAutoPlus';
    }
  ...
  function getComparableReleaseVersion(release) {
    const fallbackFamily = [VERSION_FAMILY_YANAUTOPLUS, VERSION_FAMILY_ULTRA, VERSION_FAMILY_PRO].includes(release?.family)
      ? release.family
  ...
  function setIgnoredUpdateVersion(version) {
    const normalized = formatDisplayVersion(version, VERSION_FAMILY_YANAUTOPLUS) || String(version || '').trim();
    try {
      if (normalized) {
  ...
  function getLocalVersionLabel(manifest = chrome.runtime.getManifest()) {
    const versionName = formatDisplayVersion(manifest?.version_name, VERSION_FAMILY_YANAUTOPLUS);
    if (versionName) {
      return versionName;
    }

    const versionCore = extractVersionCore(manifest?.version || '');
    return versionCore ? formatDisplayVersion(`YanAutoPlus ${versionCore}`, VERSION_FAMILY_YANAUTOPLUS) : '';
  }

  async function getReleaseSnapshot(options = {}) {
    const localVersion = getLocalVersionLabel(chrome.runtime.getManifest()) || 'YanAutoPlus 0.0';
