(function attachBackgroundPhoneVerification(root, factory) {
  ...
  function createPhoneVerificationHelpers(deps = {}) {
    ...
    const DEFAULT_PHONE_SMS_PROVIDER = normalizePhoneSmsProvider(depsDefaultPhoneSmsProvider || PHONE_SMS_PROVIDER_SMSPOOL);
    const DEFAULT_PHONE_SMS_PROVIDER_ORDER = Object.freeze(
      normalizePhoneSmsProviderOrder(depsDefaultPhoneSmsProviderOrder, [
    ...
    function normalizePhoneSmsProvider(value = '') {
      ...
      if (normalized === PHONE_SMS_PROVIDER_HERO) {
    ...
    function resolvePhoneProviderOrder(state = {}, preferredProvider = '') {
      ...
      if (hasExplicitOrder) {
        ...
        if (explicitOrder.length) {
          return [currentProvider, ...explicitOrder.filter((provider) => provider !== currentProvider)]
            .slice(0, 3);
        }
    ...
    function createResolvedSmsPoolProvider() {
      const rootScope = typeof self !== 'undefined' ? self : globalThis;
      const factory = createSmsPoolProvider
        || rootScope.PhoneSmsSmsPoolProvider?.createProvider
        || rootScope.PhoneSmsPoolProvider?.createProvider;
      if (typeof factory !== 'function') {
        return null;
      }
      return factory({
    ...
    function resolveCountryConfigFromActivation(activation, fallbackState = {}) {
      ...
      if (activation && typeof activation === 'object') {
        if (providerId === PHONE_SMS_PROVIDER_SMSPOOL) {
          const countryId = String(activation.countryId || '').trim().toUpperCase();
          if (countryId) {
            const matched = candidates.find((entry) => String(entry.id || '').trim().toUpperCase() === countryId);
            if (matched) {
              return matched;
            }
            return {
              id: countryId,
              label: String(activation.countryLabel || countryId).trim() || countryId,
            };
          }
        } else
