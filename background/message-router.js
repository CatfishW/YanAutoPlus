  ...
  function createMessageRouter(deps = {}) {
    ...
    function normalizePreStartSkippedNodeIds(value = [], state = {}) {
      const validNodeIds = typeof getNodeIdsForState === 'function'
        ...
        : Object.keys(state?.nodeStatuses || {});
    ...
    async function handleMessage(rawMessage, sender) {
      ...
      switch (message.type) {
        ...
        case 'AUTO_RUN': {
          ...
          const hasRetryNonFreeTrial = Object.prototype.hasOwnProperty.call(message.payload || {}, 'autoRunRetryNonFreeTrial');
          const hasRetryPaypalCallback = Object.prototype.hasOwnProperty.call(message.payload || {}, 'autoRunRetryPaypalCallback');
          const hasPreStartSkippedNodes = Object.prototype.hasOwnProperty.call(message.payload || {}, 'autoRunPreStartSkippedNodeIds');
          const autoRunRetryNonFreeTrial = hasRetryNonFreeTrial
            ? Boolean(message.payload?.autoRunRetryNonFreeTrial)
            : Boolean(state?.autoRunRetryNonFreeTrial);
          const autoRunRetryPaypalCallback = hasRetryPaypalCallback
            ? Boolean(message.payload?.autoRunRetryPaypalCallback)
            : Boolean(state?.autoRunRetryPaypalCallback);
          const autoRunPreStartSkippedNodeIds = normalizePreStartSkippedNodeIds(
            message.payload?.autoRunPreStartSkippedNodeIds ?? state?.autoRunPreStartSkippedNodeIds ?? [],
            state
          );
          const mode = message.payload?.mode === 'continue' ? 'continue' : 'restart';
          const autoRunStatePatch = { autoRunSkipFailures };
          const autoRunOptions = { autoRunSkipFailures, mode };
          if (hasRetryNonFreeTrial || state?.autoRunRetryNonFreeTrial !== undefined) {
            autoRunStatePatch.autoRunRetryNonFreeTrial = autoRunRetryNonFreeTrial;
            autoRunOptions.autoRunRetryNonFreeTrial = autoRunRetryNonFreeTrial;
          }
          if (hasRetryPaypalCallback || state?.autoRunRetryPaypalCallback !== undefined) {
            autoRunStatePatch.autoRunRetryPaypalCallback = autoRunRetryPaypalCallback;
            autoRunOptions.autoRunRetryPaypalCallback = autoRunRetryPaypalCallback;
          }
          if (hasPreStartSkippedNodes || Array.isArray(state?.autoRunPreStartSkippedNodeIds)) {
            autoRunStatePatch.autoRunPreStartSkippedNodeIds = autoRunPreStartSkippedNodeIds;
            autoRunOptions.autoRunPreStartSkippedNodeIds = autoRunPreStartSkippedNodeIds;
          }
          await setState(autoRunStatePatch);
          startAutoRunLoop(totalRuns, autoRunOptions);
          return { ok: true };
        }
        ...
        case 'SCHEDULE_AUTO_RUN': {
          ...
          return await scheduleAutoRun(totalRuns, {
            ...
            autoRunPreStartSkippedNodeIds: normalizePreStartSkippedNodeIds(
              message.payload?.autoRunPreStartSkippedNodeIds ?? state?.autoRunPreStartSkippedNodeIds ?? [],
              state
            ),
