...
test('sidepanel html exposes phone verification toggle and multi-provider SMS rows', () => {
  ...
  assert.match(html, /id="row-sms-pool-api-key"/);
  assert.match(html, /id="input-sms-pool-api-key"[^>]*value=""/);
  assert.match(html, /id="row-sms-pool-country"/);
  assert.match(html, /id="select-sms-pool-country"[^>]*multiple/);
  assert.match(html, /<option value="US" selected>United States<\/option>/);
  assert.match(html, /id="row-sms-pool-service"/);
...
test('sidepanel css does not hard-hide phone verification settings card', () => {
  assert.doesNotMatch(
    sidepanelCss,
...
test('sidepanel source supports SMSPool sidebar visibility in local preview', () => {
  assert.match(sidepanelSource, /function hasChromeRuntimeApi\(\)/);
  assert.match(sidepanelSource, /function buildLocalSidepanelPreviewState\(\)/);
  assert.match(
    sidepanelSource,
    /if \(!hasChromeRuntimeApi\(\)\) \{[\s\S]*buildLocalSidepanelPreviewState\(\)[\s\S]*applySettingsState\(state\);/
  );
  assert.match(sidepanelSource, /if \(hasChromeRuntimeMessageListener\(\)\) \{\s*chrome\.runtime\.onMessage\.addListener/);
  assert.match(sidepanelSource, /const showProviderConfig = canShowPhoneSettings;/);
  assert.match(sidepanelSource, /const showSmsPoolConfig = showProviderConfig;/);
  assert.match(sidepanelSource, /rowPhoneVerificationFold\.style\.display = showProviderConfig \? '' : 'none';/);
  assert.match(sidepanelSource, /rowSmsPoolApiKey\.style\.display = showSmsPoolConfig \? '' : 'none';/);
  assert.match(sidepanelSource, /loadSmsPoolCountries\(\),[\s\S]*loadHeroSmsCountries\(\),/);
  assert.match(sidepanelSource, /clickedInsideSmsPoolCountryMenu/);
  assert.match(sidepanelSource, /setSmsPoolCountryMenuOpen\(false\);/);
});

test('SMSPool provider label does not fall back to stale HeroSMS registry text', () => {
...
function getSelectedSmsPoolCountries() { return [{ id: 'US', label: 'United States' }]; }
...
return { displayHeroSmsPlatform, getPhoneSmsProviderLabel, updateHeroSmsPlatformDisplay };
`)();

  assert.equal(api.getPhoneSmsProviderLabel('smspool'), 'SMSPool');
  api.updateHeroSmsPlatformDisplay();
  assert.equal(api.displayHeroSmsPlatform.textContent, 'SMSPool / OpenAI / ChatGPT / United States');
});
...
const phoneVerificationSection = { style: { display: 'none' } };
...
const btnTogglePhoneVerificationSection = {
  ...
  const DEFAULT_PHONE_SMS_PROVIDER_ORDER = ['smspool', 'hero-sms', '5sim', 'nexsms'];
  ...
  function resolveNormalizedProviderOrderForRuntime(state = {}) {
    const fallbackProvider = String(selectPhoneSmsProvider?.value || state?.phoneSmsProvider || 'smspool').trim().toLowerCase() || 'smspool';
    ...
    if (normalizedOrder.length) {
      return [fallbackProvider, ...normalizedOrder.filter((provider) => provider !== fallbackProvider)].slice(0, 3);
    }
    return [fallbackProvider];
  }
function updatePhoneSmsProviderOrderSummary() {}
const rowSmsPoolApiKey = { style: { display: 'none' } };
const rowSmsPoolCountry = { style: { display: 'none' } };
...
return {
  ...
  rowSmsPoolApiKey,
  rowSmsPoolCountry,
  rowSmsPoolCountryFallback,
  rowSmsPoolService,
  rowSmsPoolPool,
  btnSmsPoolCountryMenu,
  btnSmsPoolCountryClear,
...
`)();
  ...
  assert.equal(api.rowSmsPoolApiKey.style.display, '');
  assert.equal(api.rowSmsPoolCountry.style.display, '');
  assert.equal(api.rowSmsPoolCountryFallback.style.display, '');
  assert.equal(api.rowSmsPoolService.style.display, '');
  assert.equal(api.rowSmsPoolPool.style.display, '');
  assert.equal(api.btnSmsPoolCountryMenu.disabled, false);
  assert.equal(api.btnSmsPoolCountryClear.disabled, false);
  ...
  api.setSelectedPhoneSmsProvider('hero-sms');
  api.setLatestState({ phoneSmsProvider: 'smspool', phoneSmsProviderOrder: ['hero-sms', '5sim'] });
  api.updatePhoneVerificationSettingsUI();
...
const rowSmsPoolApiKey = { style: { display: '' } };
const rowSmsPoolCountry = { style: { display: '' } };
...
function normalizePlusAccountAccessStrategy(value = '') {
  return String(value || '').trim().toLowerCase() === 'oauth' ? 'oauth' : 'oauth';
}
const SIGNUP_METHOD_EMAIL = 'email';
const SIGNUP_METHOD_PHONE = 'phone';
const DEFAULT_SIGNUP_METHOD = SIGNUP_METHOD_EMAIL;
