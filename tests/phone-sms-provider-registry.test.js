...
test('phone sms provider registry normalizes ids, order and labels consistently', () => {
  const registry = loadRegistry({
    ...
    PhoneSmsSmsPoolProvider: {
      createProvider: (deps = {}) => ({ provider: 'smspool', deps }),
    },
  });

  assert.deepStrictEqual(registry.getProviderIds(), ['smspool', 'hero-sms', '5sim', 'nexsms']);
  assert.equal(registry.normalizeProviderId(' SMSPOOL '), 'smspool');
  assert.equal(registry.normalizeProviderId(' NEXSMS '), 'nexsms');
  assert.equal(registry.normalizeProviderId('unknown-provider'), 'smspool');
  assert.equal(registry.getProviderLabel('smspool'), 'SMSPool');
  assert.equal(registry.getProviderDefinition('smspool').moduleKey, 'PhoneSmsSmsPoolProvider');
  ...
  assert.deepStrictEqual(
    ...
    ['nexsms', 'smspool', '5sim', 'hero-sms']
  );
  assert.deepStrictEqual(
    registry.normalizeProviderOrder([], ['nexsms', 'smspool', '5sim', 'nexsms']),
    ['nexsms', 'smspool', '5sim']
  );
  assert.deepStrictEqual(
  ...
  assert.deepStrictEqual(
    registry.createProvider('smspool', { foo: 2 }),
    { provider: 'smspool', deps: { foo: 2 } }
  );
