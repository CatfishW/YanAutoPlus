...
const DEFAULT_SMS_POOL_COUNTRY_ORDER = ['US'];
...
return {
  ...
  assert.equal(api.normalizePersistentSettingValue('phoneSmsProvider', 'SMSPOOL'), 'smspool');
  ...
  assert.equal(api.normalizePersistentSettingValue('phoneSmsProvider', 'unknown'), 'smspool');
  assert.deepStrictEqual(api.normalizePersistentSettingValue('phoneSmsProviderOrder', ['nexsms', 'smspool', '5sim', 'nexsms']), ['nexsms', 'smspool', '5sim']);
  assert.equal(api.normalizePersistentSettingValue('smsPoolApiKey', ' pool-key '), ' pool-key ');
