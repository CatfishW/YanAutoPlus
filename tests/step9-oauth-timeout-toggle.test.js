...
test('step9 completes add-phone with SMSPool phone verification before confirming OAuth', async () => {
  const api = new Function('step9ModuleSource', `
const self = {};
let webNavListener = null;
let webNavCommittedListener = null;
...
async function getStep8PageState() {
  return phoneVerificationCalls === 0
    ? {
...
const phoneVerificationHelpers = {
  async completePhoneVerificationFlow(tabId, pageState) {
    ...
    if (tabId !== 123) throw new Error('unexpected tab');
    if (!pageState?.addPhonePage) throw new Error('expected add-phone page');
    return {
      success: true,
      consentReady: true,
      url: 'https://auth.openai.com/sign-in-with-chatgpt/codex/consent',
...
`)(step9ModuleSource);
  ...
  const snapshot = api.snapshot();
  assert.equal(snapshot.phoneVerificationCalls, 1);
  assert.equal(snapshot.waitReadyCalls, 1);
  assert.equal(snapshot.cleanupCalls >= 1, true);
  assert.equal(snapshot.hasPendingReject, false);
  assert.equal(
    snapshot.logs.some((entry) => /SMSPool/.test(entry.message)),
    true
  );
  assert.deepEqual(snapshot.completePayload, {
    nodeId: 'confirm-oauth',
